...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [x_h, cov]=ZUPTaidedINS_RUPT(u,imu)
...
for i = 1:n
    ...
    [P{i}, Q{i}, R{i}, H{i}]=init_filter;          % Subfunction located further down in the file.

    % Allocate vecors
    [x_h{i}, cov{i}, Id{i}]=init_vec(N,P{i});     % Subfunction located further down in the file.

    % Initialize the navigation state vector x_h, and the quaternion vector
    % quat.
    [x_h{i}(1:9,1), quat{i}]=init_Nav_eq(u{i},imu{i}.x0);  % Subfunction located further down in the file.
...
for k=2:N
    ...
    for i = 1:n
        ...
        % sensor errors  
        u_h=comp_imu_errors(u{i}(:,k),x_h{i}(:,k-1)); % Subfunction located further down in the file.


        % Update the navigation equations.   
        [x_h{i}(:,k), quat{i}]=Navigation_equations(x_h{i}(:,k-1),u_h,quat{i}); % Subfunction located further down in the file.


        % Update state transition matrix
        [F, G]=state_matrix(quat{i},u_h); % Subfunction located further down in the file.
        ...
        % following
        if imu{i}.zupt(k)==true;

            % Calculate the Kalman filter gain
            K=(P{i}*H{i}')/(H{i}*P{i}*H{i}'+R{i});
            ...
            [x_h{i}(:,k), quat{i}]=comp_internal_states(x_h{i}(:,k),dx,quat{i});     % Subfunction located further down in the file.
    ...
    % update every RUPT_rate seconds
    if mod(k*simdata.Ts,simdata.RUPT_rate) == 0
        foot_origin = zeros(3,1);
        foot_orientation = zeros(3,1);
        for i = 1:n
            foot_origin = foot_origin + (x_h{i}(1:3,k) - eul2rotm(x_h{i}([6 5 4],k)')*imu{i}.p)./n;
            foot_orientation = foot_orientation + x_h{i}(4:6,k)./n;
        end
        
        for i = 1:n
            x_h{i}(1:3,k) = foot_origin + eul2rotm(foot_orientation([3 2 1])')*imu{i}.p;
            x_h{i}(4:6,k) = foot_orientation;
        end
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [x, quat]=init_Nav_eq(u,x0)
