...
%simdata.init_pos=[0 0 0]'; <--- Initial position is IMU dependent
...
simdata.gamma=0.5e5; %1e5 or 5e5

% RUPT update rate in seconds
simdata.RUPT_rate = 5; % [sec]
...
simdata.sigma_initial_acc_bias=0.0001*ones(3,1);           % Accelerometer biases (x,y,z platform coordinate axis)[m/s^2]
simdata.sigma_initial_gyro_bias=0.0001*pi/180*ones(3,1);   % Gyroscope biases (x,y,z platform coordinate axis) [rad/s]                               
...
% global simdata;
% fi = [simdata.path 'data_inert.txt']
% 
% % Load inertial data
